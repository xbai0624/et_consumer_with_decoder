...
{
    std::cout<<"---------------------------------------------"<<std::endl;
    std::cout<<"event length from event parser: "<<fBufLen<<std::endl;
    for(int i=0; i<fBufLen; i++)
        std::cout<<"0x"<<std::hex<<std::setw(10)<<std::setfill(' ')<<ntohl(pBuf[i])<<std::endl;
    ...
    {
        std::cout<<"Undefined bank type.."<<std::endl;
    }
...
{
    std::cout<<"parsing bank..."<<std::endl;
    EventBankHeader event_header(pBuf[0], pBuf[1]);
...
{
    std::cout<<"parsing segments..."<<std::endl;
...
{
    std::cout<<"parsing tag segment..."<<std::endl;
        ...
        std::vector<int> &vTagTrack)
{
    ...
    switch(content_type) 
    {
        ...
        case EvioPrimitiveDataType::Undefined:
            std::cout<<"debug: undefined evio primitive data type."<<std::endl;
            // unsupported evio data type
            break;
        default:
            {
                std::cout<<"debug: parsing data bank..."<<std::endl;
        ...
        std::vector<int> &vTagTrack)
{
    ...
    if(mDecoder.find(tag) != mDecoder.end()) {
        std::cout<<"debug: found bank: "<<tag<<std::endl;
        mDecoder[tag] -> Decode(&pBuf[header_length], length-1, vTagTrack);
    }
    else {
        std::cout<<"bank not found: "<<tag<<std::endl;
    }
}
